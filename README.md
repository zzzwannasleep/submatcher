...
TV / Web 的字幕搬到 BD 上，时间轴对不上。Sushi 靠音频对齐，SubMatcher 两样都用：先像 Sushi 那样按声音给每行粗定位，再把这行出现时的画面拿到新片源里、在附近找准到帧。CC / 放送版多出或删掉片段、某段画面对不上时，按声音走，不会错沿用邻近行。台标、分辨率、调色不同都不影响，音轨重混或者干脆没有音轨时自动只用画面。
